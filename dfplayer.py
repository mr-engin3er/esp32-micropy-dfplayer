...
class Player():
    ...
    def command(self, CMD, Par1, Par2):
        ...
        high_byte, low_byte = split(checksum)
        command_line = bytes([b & 0xFF for b in [
            start_byte, version_byte, command_length, CMD, acknowledge,
            Par1, Par2, high_byte, low_byte, end_byte
        ]])
    ...
    def play(self, folder_num, track_num):
        self.awaitconfig()
        self.playtime = ticks_ms()
        self.command(0x0F, folder_num, track_num)

    def finish(self, folder_num, track_num):
        self.play(folder_num, track_num)
        while self.playing():
            sleep_ms(50)
